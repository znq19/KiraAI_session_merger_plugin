...
## 合并排序模式：time vs session_blocks —— 上下文怎么摆？选哪个更好？

两种模式**看到的内容完全一样**（都是全局按时间竞争，保留最近 N 轮，N=最大合并轮数，超限时=保留轮数），区别只在**怎么排列**。

| | **time（默认）** | **session_blocks** |
|---|---|---|
| 排列 | 所有会话按时间交错成一条线 | 按会话分块：他会话块在前、当前会话块在最后 |
| 读起来像 | 完整人生时间线，现场感强 | 分章节的日记：「群里三章 + 私聊十章」 |
| 前缀缓存 | 实测两者命中率接近，不构成选择依据 | 同左 |

**怎么选**：
- **默认 time**：bot 能强烈感到「我刚才就在那个群里出现过」，多会话接龙、角色扮演沉浸感最好，大多数时候直接用这个即可
- 喜欢「分账本」式清晰分区、或ta自己试出某种模式更顺 → session_blocks（跨会话记忆同样完整，靠 `[session: xxx]` 来源前缀 + 「当前会话约束」提示词告诉模型哪些是别个会话的事）
- 拿不准就根据具体的bot表现和命中缓存率，和发 `/mpreview` 看看两种排列的实际样子再决定。
...
| 项目 | 默认 | 说明 |
|------|------|------|
| 合并排序模式 | time | 时间交错（现场感强）/ session_blocks 分块排版；看到的内容相同，实测缓存命中率接近 |
| 最大合并轮数 | 10 | 全局保留最近 N 轮 |
...
- **修复（2.6.7 引入的回归）**：`"[转发消息]"` / `"[引用]"` 是**真实段的渲染结果**
  （转发卡片 / 无 id 的引用），不是占位。2.6.7 把它们放进占位集合、又加了"按段渲染
  二次判定"，导致**转发消息整条被过滤**、msg_id 丢失，bot 无法再转发嵌套转发。
- **修复**：占位集合只保留 `{"[引用消息]", "[空消息]"}`，且只对 **`user_id=0` 的合成
  占位行**生效；真实用户消息一律保留（即使正文恰好是 `[引用消息]`）。
- **增强**：转发段在 raw_message 缺失时渲染为 `[转发消息](id=<resId>)`。
- **测试**：`tests/test_history_filter.py` 扩到 13 项。
...
- **session_blocks 语义重设计：全局选择 + 分块排版**。选择与 time 完全一致——所有成员按时间全局竞争、保留最近 N 轮（N=`max_merged_chunks`，超限时=`keep`），当前会话无特权、活跃会话自然占大头；排版保持分块——他人块 sid 字典序在前、当前会话块恒在最后。两种模式看到的内容相同，只是排列不同
- **默认排序模式改为 time**（实测两种模式缓存命中率接近，时序现场感更适合多数场景）
- 取代 2.5.2 的「每块独立预算」方案：全局预算下块占几轮由时间竞争决定，可能为 0（与 time 相同）
- `preview` 预览同步为新语义

### 2.5.2

- **修复 session_blocks 模式合并失效**：其他会话块排在前、当前会话块在后，但三处截断（`trim_chunks` / 未超限 `apply_round_cap` / 超限 keep 截断）都是全局尾部保留——当前会话轮数达到上限时，其他会话块被整体丢光，合并名存实亡（默认配置即中招）。改为**按会话块各自截断再拼接**（未超限按 `max_merged_chunks`/块、超限按 `keep`/块），块顺序与前缀缓存设计不变；time 模式行为不变
- `preview` 命令改走与实际合并完全相同的构建路径，预览即所见
